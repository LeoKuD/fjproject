        ...
        <div class="collapse" id='test${index}'>
          <div class="question">
            ...
            <div class="question__list">
              ${questionStatistics.map( ({avgProc}, index) => {
                return `
                  <div class="mt-3 question__item">
                    <span class="px-2">${index + 1}.</span><textarea class="py-2 px-4 question__name" rows="1" readonly>jsfkfskjsjdkfsjk</textarea><span></span><span class="text-center">${avgProc}%</span>
                  </div>
                `
              }).join('')}
            </div>
...
themeSelect.addEventListener('change', async ({ target }) => {
  ...
  try {
    ...
    const url = new URL("http://getTestsStatistic");
